    ...
    if (chatSendBtn) {
        chatSendBtn.onclick = async () => {
            ...
            try {
                ...
                const msg = err && err.message ? `Error: ${err.message}` : 'Unable to get server response.';
                renderError(msg, () => {
                    chatSendBtn.click();
                });
            } finally {
        ...
        link.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (endpoint === 'profile') {
                    ...
                    const summaryLines = data.summary?.en || [];
                    html += `
                        <div style="line-height:1.6;">
                            <p><strong>Name:</strong> ${info.full_name || 'N/A'}</p>
                            ${info.email ? `<p><strong>Email:</strong> ${info.email}</p>` : ''}
                            ${location.city ? `<p><strong>Location:</strong> ${location.city}, ${location.region || ''}, ${location.country || ''}</p>` : ''}
                            ${summaryLines.length > 0 ? `<div style="margin-top:15px;"><strong>Summary:</strong><ul style="margin-top:10px;">${summaryLines.map(line => `<li style="margin-bottom:8px;">${line}</li>`).join('')}</ul></div>` : ''}
                        </div>
                    `;
                } else if (endpoint === 'skills') {
                    const technical = data.technical?.en || [];
                    const soft = data.skills?.en || [];
                    
                    if (technical.length > 0) {
                        html += '<h3 style="color:#4a90e2;margin-top:20px;">Technical Skills</h3>';
                    ...
                    if (soft.length > 0) {
                        html += '<h3 style="color:#4a90e2;margin-top:20px;">Soft Skills</h3>';
                    ...
                    if (Array.isArray(data)) {
                        data.forEach(exp => {
                            const responsibilities = exp.responsibilities_en || [];
                            html += `
                                <div style="margin-bottom:20px;padding:15px;background:#f9f9f9;border-radius:8px;">
                                    <h3 style="margin:0 0 10px 0;color:#4a90e2;">${exp.role_en || exp.role_es || 'Position'}</h3>
                        ...
                        html += '<p>No experience available.</p>';
                    }
                } else if (endpoint === 'education') {
                    if (Array.isArray(data)) {
                        data.forEach(edu => {
                            const focus = edu.focus_en || [];
                            html += `
                                <div style="margin-bottom:20px;padding:15px;background:#f9f9f9;border-radius:8px;">
                                    <h3 style="margin:0 0 10px 0;color:#4a90e2;">${edu.degree_en || edu.degree_es || 'Degree'}</h3>
                                    ...
                                    ${focus.length > 0 ? `<p style="margin:10px 0 0 0;font-size:0.95em;"><strong>Focus:</strong> ${focus.join(', ')}</p>` : ''}
                                </div>
                            `;
                        });
                    } else {
                        html += '<p>No education available.</p>';
                    }
                    ...
                    if (Array.isArray(data)) {
                        data.forEach(pub => {
                            ...
                            html += `
                                <div style="margin-bottom:20px;padding:15px;background:#f9f9f9;border-radius:8px;">
                                    <h3 style="margin:0 0 10px 0;color:#4a90e2;">${pub.title_en || pub.title_es || 'Publication'}</h3>
                                    ${pub.description_en ? `<p style="margin:10px 0;font-size:0.95em;line-height:1.6;">${pub.description_en}</p>` : ''}
                                    ${details.journal ? `<p style="margin:5px 0;"><strong>Journal:</strong> ${details.journal}</p>` : ''}
                                    ${details.year ? `<p style="margin:5px 0;"><strong>Year:</strong> ${details.year}</p>` : ''}
                                    ${details.coauthor ? `<p style="margin:5px 0;"><strong>Co-author:</strong> ${details.coauthor}</p>` : ''}
                                    ${details.url ? `<p style="margin:10px 0;"><a href="${details.url}" target="_blank" style="color:#4a90e2;text-decoration:none;">➡ View publication</a></p>` : ''}
                                </div>
                            `;
                        });
                    } else {
